...
- 1단계: 이름·생년월일·연락처·이메일(전체)·입사일(선택)·주소·직급·팀·비밀번호·개인정보 동의 입력
  - 이메일: 전체 주소 입력 (`allowed_emails` 사전 등록된 이메일만 가입 가능)
  - 생년월일·입사일: 숫자 입력 → `.` 자동 삽입(YYYY.MM.DD 표시), hidden 필드로 YYYYMMDD 전달
...
### 대시보드 (`/dashboard`)
- 템플릿: `dashboard.html`
- 역할별 메뉴 그리드 표시 (버튼 먼저, 통계 카드 아래)
- 본인 정보(이름·직급·팀·역할) 표시
- 내 프로젝트 현황 (모든 역할): ACTIVE·PLANNED 배정 목록, 상태 배지·기간·잔여일
- SALES/ADMIN 전용 KPI 4종 (현재 투입 중·이번달 신규·이번달 종료·미투입) + 종료 임박 5건
- ADMIN 빠른 메뉴: 관리자 패널·통계·재직증명서·허용 이메일 4개
...
- 2컬럼 레이아웃: 좌측 등록 영역(단건·텍스트 일괄·엑셀 업로드), 우측 목록(검색 필터 포함)
- 단건 등록 / 텍스트 일괄(쉼표·줄바꿈 구분) / 엑셀(.xlsx/.xls, 첫 번째 열) 등록
- 목록 내 실시간 검색 필터 (JS `filterList()`)
- 개별 삭제 버튼 (confirm 확인)

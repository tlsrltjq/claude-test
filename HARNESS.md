...
**현재 상태 (2026-06-08):** Flyway V230(V209 삭제). BUILD SUCCESSFUL. Docker 로컬 환경 구성 완료(PostgreSQL 18 + 앱 컨테이너). Playwright 브라우저 테스트 19/19 전 통과 확인. CSP 인라인 이벤트 핸들러 전면 수정 완료(20개 템플릿, 8a631dc). 미결: 인력표 팀 미배정자 노출 여부(현재 포함 상태, 정책 결정 필요). 백로그: 고정 도메인 구매·적용, NAS 저장소 연동.

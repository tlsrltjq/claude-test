...
cat <<EOF
=== eActive Resource Hub — Stage $STAGE_ID ===

[프로젝트 SSOT]
- 위치: $MVP_ROOT/docs/PROJECT_SPEC.md
- 핵심 제약 (위반 금지):
  ...
  - 코드는 $WORKSPACE_ROOT/eactive-resource-hub/ 아래에 생성 (워크스페이스 루트에 위치)

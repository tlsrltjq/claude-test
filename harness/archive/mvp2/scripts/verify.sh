...
#   bash harness/scripts/verify.sh 01-permissions
#   bash harness/scripts/verify.sh all                # MVP2 1~10 전부
#   bash harness/scripts/verify.sh all --with-mvp1    # + MVP1 회귀 검사
#   bash harness/scripts/verify.sh 04 --with-mvp1     # 04 + MVP1 회귀 검사
...
run_one() {
  ...
  local stage_dir=""
  local num
  # 10#$arg → 강제 10진수 해석 (08, 09가 잘못된 octal로 처리되는 문제 방지)
  num=$(printf "%02d" "$((10#${arg%%[^0-9]*}))" 2>/dev/null || echo "")
  if [ -n "$num" ]; then
